...
- **📊 場次總覽與名單管理**：
  - 場次進度顏色標記：**🟢 深綠色（已額滿）** / **🟡 淺綠色（招募中）** / **⚪ 灰色（已停用）**。
  - **修改場次內容**：未開始之場次可隨時調整時間、地點、用球、費用與人數上限（防呆不可低於已報名人數）。
  - **🚫 停用與重新啟用場次**：遇天候或臨時變卦可一鍵暫停報名（名冊完整保留），狀況排除隨時恢復開放。
  - **🗑️ 永久刪除場次**：支援團主或 Super Admin 徹底清除測試場次，並原子級刪除該場次所有報名與候補資料。
  - **🧹 過期歷史場次自動清理**：預設在場次結束 7 天後自動從資料庫釋放過期資源（可透過環境變數自訂）。
...
- **🏢 社團場次隔離政策 (`ALLOW_CROSS_GROUP_SESSIONS`)**：
  - **嚴格封閉隔離模式（預設）**：各羽球社團開立之專屬場次僅限該群球友報名，跨群或私訊無法窺探其他社團名額，並以社團標籤清楚辨別場次歸屬。
  - **全域開放撮合模式（選用）**：可透過環境變數一鍵切換為公開撮合平台，開放所有球友自由跨群瀏覽與報名全站開放場次。
...
<details>
<summary><b>📜 點此展開閱讀 MIT 授權條款繁體中文參考譯本</b></summary>

> **備註**：本中文譯本僅供閱讀與理解之便利，若文義理解有任何歧異，概以 [LICENSE](LICENSE) 英文正本為準。
...
茲免費授權予任何取得本軟體及相關說明文件（下稱「軟體」）副本之人，得無限制處理該軟體，
包括但不限於享有使用、複製、修改、合併、發布、散布、再授權及／或販售軟體副本之權利，
並允許獲提供該軟體之人享有相同權利，但須符合下列條件：

上述版權聲明及本授權聲明，應包含於本軟體的所有副本或重大實質部分中。

本軟體係依「現狀」提供，不具備任何明示或默示之保證，包括但不限於適銷性、
特定用途適用性及未侵害他人權利之保證。在任何情況下，無論是基於契約、侵權
或其他訴訟途徑，因本軟體或本軟體之使用或其他處置所衍生或相關之任何索賠、
損害或其他責任，作者或版權持有人均不承擔任何責任。
```
</details>
